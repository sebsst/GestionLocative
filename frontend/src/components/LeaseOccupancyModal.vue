<template>
  <Modal
    ...
    size="lg"
    :show-close="true"
    :hide-footer="!(activeTab === 'history' || activeTab === 'tenants')"
  >
    ...
    <div v-if="activeTab === 'tenants'">
      ...
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold">Locataires du bail</h3>
...
watch(() => props.show, async (newValue) => {
  if (newValue && props.lease && props.lease.id) {
    activeTab.value = 'history'; // Ensure we show the Périodes tab
    await loadPeriods();
