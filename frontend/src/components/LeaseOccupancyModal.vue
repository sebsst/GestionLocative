<template>
  <div v-if="show" class="modal modal-open modal-with-tabs" @click.self="close">
    <div class="modal-box max-w-4xl">
      <h3 class="font-bold text-lg mb-2">Gestion des périodes d'occupation</h3>
        ...
        <div>
         ...
         <div class="tabs tabs-boxed mb-4 gap-1">
          <a
            ...
            :class="{ 'tab-active': activeTab === 'new', 'border-primary': activeTab === 'new' }"
            @click="activeTab = 'new'"
          >
            Nouvelle période
          </a>
          <a
            class="tab tab-lg border-2 border-base-300 rounded-lg"
            :class="{ 'tab-active': activeTab === 'tenants', 'border-primary': activeTab === 'tenants' }"
              ...
              <td>
               ...
               <td>
                 <div class="flex gap-2">
                   ...
                   <button @click="addTenantToPeriod(period)" class="btn btn-ghost btn-xs text-success" title="Ajouter un locataire">
                     <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                       <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                     </svg>
                   </button>
                   <button @click="removeTenantFromPeriod(period)" class="btn btn-ghost btn-xs text-warning" title="Retirer un locataire">
                     <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="activeTab === 'tenants'">
        <div class="mb-4">
          <button @click="showAddTenantModal = true; loadTenants();" class="btn btn-primary btn-sm">
                ...
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <div class="flex items-center gap-2">
                      ...
                      <span v-if="relation.isPrimary" class="badge badge-primary badge-sm">Principal</span>
                    </div>
                    <p class="text-sm opacity-70">{{ relation.Tenant?.email }}</p>
        ...
        <form @submit.prevent="savePeriod" class="space-y-4">
          ...
          <div v-if="error" class="alert alert-error">
           ...
           <div class="modal-action">
             <button type="button" @click="close" class="btn btn-ghost">Annuler</button>
     ...
     >
       ...
       <template #footer>
         <div class="flex justify-end gap-2">
           ...
           <button @click="saveTenantToPeriod" class="btn btn-primary">
...
const saveTenantToPeriod = async () => {
  ...
  try {
    ...
    // Update occupancy period: end current period and create new one
    try {
